...
def generate_reflex(base_dir, protocols)
  ...
  protocols.each do |protocol|
    ...
    protocol.interfaces.each do |interface|
      ...
      open("#{base_dir}/reflex/src/protocol/#{protocol.name}/#{interface.name}/lib.rs", 'wb') do |f|
        ...
        f.puts(<<~CODE)
          ...
          #[allow(dead_code)]
        CODE
        f.puts("pub fn dispatch_request(#{interface.dispatch_context_mut}context: crate::protocol::session::Context<#{interface.receiver_type}>, opcode: u16, args: Vec<u8>) -> Box<dyn futures::future::Future<Item = crate::protocol::session::Session, Error = ()> + Send> {")
    ...
    protocol.interfaces.each do |interface|
      open("#{base_dir}/reflex/src/protocol/#{protocol.name}/#{interface.name}.rs", 'wb') do |f|
        ...
        USE
        f.puts('pub mod enums;') if interface.enums
        f.puts('pub mod events;') if interface.events
        f.puts('mod lib;')
        f.puts('pub use lib::*;')
        f.puts('')
